...
//Definations
bool InjErr=1;
char *TrgtOpCodeName="add";
//enum TargetInstruction {ADD=8, SUB=10, MULTIPLY, AND, OR, XOR};
enum TargetField {OPCODE, DESTINATION};
//Setting Options
//Enable Error injection
//InjErr=true;
//TargetInstruction TrgtInst=ADD;
TargetField TrgtField=OPCODE;
//*******************************************************
...
#define FLOAT_VECTOR_OP(OP) {                                         \
  if (dyn_cast<VectorType>(Ty)->getElementType()->isFloatTy())        \
    ...
    case Instruction::Sub:   R.IntVal = Src1.IntVal - Src2.IntVal; break;//commented by naveed
    //case Instruction::Sub:   R.IntVal = Src1.IntVal + Src2.IntVal; break;//added by Naveed
...
//InjErr=true;
//TargetInstruction TrgtInst=ADD;
//TargetField TrgtField=DESTINATION;

//errs()<<"Target Opcode name ="<<TrgtOpCodeName<<"\n";
//errs()<<"I.getOpcodeName() ="<<I.getOpcodeName()<<"\n";

//if error injection is enabled and current Opcode is the target Opcode for error injection
if (InjErr && (TrgtOpCodeName==I.getOpcodeName()) )
	{
	int ToggleBit;
	unsigned OpCode_var;
	int Mask=1;
	switch(TrgtField)//switch on the field of instruction to be injected with errors
		{
		case OPCODE:
			OpCode_var=I.getOpcode();//get the opcode of instruction in a variable
			errs() << "Before toggling OpCode_var="<<OpCode_var<<"\n";
			srand (time(0));
			ToggleBit=rand()%(R.IntVal.getBitWidth());//randomly selecet a single bit to be flipped
			errs() << "ToggleBit ="<<ToggleBit<<"\n";
			Mask=Mask<<ToggleBit;//Build a mask for toggling
			errs() << "Mask ="<<Mask<<"\n";
			OpCode_var^=Mask;///Perform the actual toggling of selected bit position
			errs() << "After toggling OpCode_var="<<OpCode_var<<"\n";
			switch (OpCode_var) //Now execute the operation indicated by single-bit flipped opcode.
				{
				default:
			        dbgs() << "Illegal corrupted opcode!\n-->" << I;
      				llvm_unreachable(0);
      				break;
    				case Instruction::Add:   errs() << "Add\n";R.IntVal = Src1.IntVal + Src2.IntVal; break;
    				case Instruction::Sub:   errs() << "Sub\n";R.IntVal = Src1.IntVal - Src2.IntVal; break;  					case Instruction::Mul:   errs() << "Mul\n";R.IntVal = Src1.IntVal * Src2.IntVal; break;
    				case Instruction::FAdd:  errs() << "FAdd\n";executeFAddInst(R, Src1, Src2, Ty); break;
    				case Instruction::FSub:  errs() << "FSub\n";executeFSubInst(R, Src1, Src2, Ty); break;
    				case Instruction::FMul:  errs() << "FMul\n";executeFMulInst(R, Src1, Src2, Ty); break;
    				case Instruction::FDiv:  errs() << "FDiv\n";executeFDivInst(R, Src1, Src2, Ty); break;
    				case Instruction::FRem:  errs() << "FRem\n";executeFRemInst(R, Src1, Src2, Ty); break;
    				case Instruction::UDiv:  errs() << "UDiv\n";R.IntVal = Src1.IntVal.udiv(Src2.IntVal); break;
    				case Instruction::SDiv:  errs() << "SDiv\n";R.IntVal = Src1.IntVal.sdiv(Src2.IntVal); break;
    				case Instruction::URem:  errs() << "URem\n";R.IntVal = Src1.IntVal.urem(Src2.IntVal); break;
    				case Instruction::SRem:  errs() << "Srem\n";R.IntVal = Src1.IntVal.srem(Src2.IntVal); break;
    				case Instruction::And:   errs() << "And\n";R.IntVal = Src1.IntVal & Src2.IntVal; break;
    				case Instruction::Or:    errs() << "Or\n";R.IntVal = Src1.IntVal | Src2.IntVal; break;
    				case Instruction::Xor:   errs() << "Xor\n";R.IntVal = Src1.IntVal ^ Src2.IntVal; break;
    				}
			break;
		case DESTINATION:
			srand (time(0));
			ToggleBit=rand()%(R.IntVal.getBitWidth());//randomly generate the bit-position for single error
			errs() << "ToggleBit ="<<ToggleBit<<"\n";
			errs() << "Befor toggling R.IntVal="<<R.IntVal<<"\n";
			R.IntVal.flipBit(ToggleBit);//Toggle that bit of result and thats all
			errs() << "After toggling R.IntVal="<<R.IntVal<<"\n";
			break;
		default:
			errs() << "nothing\n";
...
if(I.getOpcode()==8)
	{
	//R.IntVal=10;
	int ToggleBit=rand()%(R.IntVal.getBitWidth());//randomly generate the bit-position for single error
	errs() << "ToggleBit ="<<ToggleBit<<"\n";
	//int Mask=1<<ToggleBit;
	//int R_Value=R.IntVal.getHiBits(R.IntVal.getBitWidth());
	//errs() << "Mask ="<<Mask<<"\n";
	errs() << "R.IntVal="<<R.IntVal<<"\n";
	R.IntVal.flipBit(ToggleBit);//Toggle that bit of result and thats all
	errs() << "R.IntVal_toogle="<<R.IntVal<<"\n";
	//errs() << "Coruupted R.Intval ="<<(R_Value^Mask)<<"\n";
	//R.IntVal=randomNum;
	}
*/
//****************************************  
...
void Interpreter::visitUnreachableInst(UnreachableInst &I) {
  errs() << "visitUnreachableInst\n";//Naveed
  report_fatal_error("Program executed an 'unreachable' instruction!");
}

void Interpreter::visitBranchInst(BranchInst &I) {
errs() << "visitVisitBranchInst\n";//Naveed
  ExecutionContext &SF = ECStack.back();
  BasicBlock *Dest;
...
void Interpreter::run() {
  while (!ECStack.empty()) {
    ...
    //errs() << "About to interpret Instruction: " << I<<"\n";//Naveed
    //errs()<<"Instruction Opcode ="<<I.getOpcode()<<"\n";//Naveed
    //errs()<<"Instruction Opcode Name ="<<I.getOpcodeName()<<"\n";//Naveed
...
//****************************************
/*
    GenericValue R_dummy=10;
    SetValue(&I, R_dummy, SF);
*/
